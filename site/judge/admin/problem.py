...
from judge.models import LanguageLimit, Problem, ProblemGroup, ProblemClarification, ProblemTranslation, Profile, Solution
...
class ProblemForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for group in groups:
            if group.children.all().count() == 0:
                selectableGroups.append(group.id)

        self.fields['group'].queryset = ProblemGroup.objects.filter(id__in=selectableGroups)
    ...
    class Meta:
        widgets = {
            ...
            'hints': AdminMartorWidget(attrs={'data-markdownfy-url': reverse_lazy('problem_preview')}),
        }
...
class ProblemTranslationInline(admin.StackedInline):
    ...
    fields = ('language', 'name', 'summary', 'description', 'hints')
...
class ProblemAdmin(NoBatchDeleteMixin, VersionAdmin):
    fieldsets = (
        (None, {
            'fields': (
                'name', 'is_public', 'date', 'authors', 'uploader',
                'summary', 'description', 'hints',
            ),
        }),
        (_('Social Media'), {'classes': ('collapse',), 'fields': ('og_image',)}),
        (_('Taxonomy'), {'fields': ('difficulty', 'types', 'group', )}),
        (_('Points'), {'fields': (('points', 'partial'),)}),
        (_('Limits'), {'fields': ('time_limit', 'memory_limit')}),
        (_('Language'), {'fields': ('allowed_languages',)}),
        (_('Justice'), {'fields': ('banned_users',)}),
        (_('History'), {'fields': ('change_message',)}),
    )
    list_display = ['code', 'name', 'show_authors', 'show_uploader', 'points', 'is_public', 'show_public']
    ordering = ['code']
    search_fields = ('code', 'name', 'authors__user__username')
    readonly_fields = ('points', 'partial', 'uploader')
    ...
    def show_uploader(self, obj):
        if obj.uploader:
            return obj.uploader.username
        else:
            return None

    show_uploader.short_description = _('Uploader')
    ...
    def save_model(self, request, obj, form, change):
        if not change:
            obj.uploader = request.user.profile
