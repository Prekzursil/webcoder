...
class PastContestProblemInlineForm(ModelForm):
    class Meta:
        widgets = {'problem': AdminHeavySelect2Widget(data_view='problem_select2')}

class PastContestProblemInline(admin.TabularInline):
    model = PastContestProblem
    verbose_name = _('Problem')
    verbose_name_plural = 'Problems'
    fields = ('problem',)
    form = PastContestProblemInlineForm

class PastContestAdmin(admin.ModelAdmin):
    fields = ('name', 'level', 'difficulty', 'year')
    list_display = ('name', 'level', 'difficulty', 'year')
    inlines = [PastContestProblemInline]