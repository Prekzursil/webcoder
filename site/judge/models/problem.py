...
__all__ = ['ProblemGroup', 'ProblemGroupTranslation', 'ProblemType', 'Problem', 'ProblemTranslation', 'ProblemClarification',
...
class ProblemType(models.Model):
    ...
    class Meta:
        ordering = ['full_name']
        verbose_name = _('problem tag')
        verbose_name_plural = _('problem tags')


class ProblemGroup(MPTTModel):
    name = models.CharField(max_length=20, verbose_name=_('problem group ID'), unique=True)
    full_name = models.CharField(max_length=40, verbose_name=_('problem group name'))
    parent = TreeForeignKey('self', on_delete=models.CASCADE, verbose_name=_('parent group'),
                              help_text=_('Select a parent group, if applicable'), null=True, blank=True, related_name='children')

    def __str__(self):
        if self.parent:
            return self.parent.full_name + ' > ' + self.full_name
        else:
            return self.full_name

    def get_group_problem_count(self):
        sum = 0
        for subgroup in self.children.all():
            sum += Problem.objects.filter(group__id=subgroup.id).count()
        return sum

    def get_problem_count(self):
        return Problem.objects.filter(group__id=self.id).count()

    def get_subgroups_count(self):
...
class ProblemGroupTranslation(models.Model):

    group = models.ForeignKey(ProblemGroup, verbose_name=_('problem'), related_name='translations', on_delete=CASCADE)
    language = models.CharField(verbose_name=_('language'), max_length=7, choices=settings.LANGUAGES)
    full_name = models.CharField(max_length=40, verbose_name=_('problem group name'))

    class Meta:
        unique_together = ('group', 'language')
        verbose_name = _('problem group translation')
        verbose_name_plural = _('problem group translations')

class TranslatedProblemGroupQuerySet(SearchQuerySet):
    def __init__(self, **kwargs):
        super(TranslatedProblemGroupQuerySet, self).__init__(('id', 'full_name'), **kwargs)

    def add_i18n_full_name(self, language):
        queryset = self._clone()
        alias = unique_together_left_join(queryset, ProblemGroupTranslation, 'group', 'language', language)
        return queryset.annotate(i18n_full_name=Coalesce(RawSQL('%s.full_name' % alias, ()), F('full_name'),
...
class TranslatedProblemQuerySet(SearchQuerySet):
    ...
    def add_i18n_name(self, language):
        ...
        return queryset.annotate(i18n_name=Coalesce(RawSQL('%s.name' % alias, ()), F('name'),
                                                    output_field=models.CharField()))
    def add_i18n_summary(self, language):
        queryset = self._clone()
        alias = unique_together_left_join(queryset, ProblemTranslation, 'problem', 'language', language)
        return queryset.annotate(i18n_summary=Coalesce(RawSQL('%s.summary' % alias, ()), F('summary'),
                                                    output_field=models.CharField()))
...
class TranslatedProblemForeignKeyQuerySet(QuerySet):
    ...
    def add_problem_i18n_summary(self, key, language, name_field=None):
        queryset = self._clone() if name_field is None else self.annotate(_summary=F(name_field))
        alias = unique_together_left_join(queryset, ProblemTranslation, 'problem', 'language', language,
                                          parent_model=Problem)
        # You must specify name_field if Problem is not yet joined into the QuerySet.
        kwargs = {key: Coalesce(RawSQL('%s.summary' % alias, ()),
                                F(name_field) if name_field else RawSQLColumn(Problem, 'summary'),
                                output_field=models.CharField())}
        return queryset.annotate(**kwargs)
...
class Problem(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name=_('problem name'), unique=True, db_index=True,
                            help_text=_('The full, unique name of the problem, '
                                        'as shown in the problem list.'))
    description = models.TextField(verbose_name=_('problem body'))
    hints = models.TextField(verbose_name=_('problem hints'),
                             help_text='These hints will be presented for the users who have at least one submission.',
                             default='',
                             blank=True)
    authors = models.ManyToManyField(Profile, verbose_name=_('authors'), blank=True, related_name='authored_problems',
    ...
    uploader = models.ForeignKey(Profile, verbose_name=_('uploader'), on_delete=SET_NULL, related_name="uploaded_problems",
                                 help_text=_('This field register automatically who uploaded the problem '
                                             'to the platform.'), null=True)
    difficulty = models.CharField(verbose_name=_('Problem difficulty'),
                                  max_length=7, choices=DIFFICULTIES)
    types = models.ManyToManyField(ProblemType, verbose_name=_('problem tags'),
                                   help_text=_('A tag for the problem, '
    ...
    summary = models.TextField(verbose_name=_('problem summary'),
                               help_text=_('Plain-text, shown in problem list and meta description tag, e.g. for social media.'))
    ...
    def i18n_summary(self):
        if self._i18n_summary is None:
            self._i18n_summary = self._trans[0].summary if self._trans else self.summary
        return self._i18n_summary

    @i18n_summary.setter
    def i18n_summary(self, value):
        self._i18n_summary = value
...
class ProblemTranslation(models.Model):
    ...
    summary = models.TextField(verbose_name=_('translated summary'))
    description = models.TextField(verbose_name=_('translated description'))
    hints = models.TextField(verbose_name=_('translated hints'), default='', blank=True)
...
class Solution(models.Model):
    ...
    class Meta:
        ...
        verbose_name = _('official source')
        verbose_name_plural = _('official source')
