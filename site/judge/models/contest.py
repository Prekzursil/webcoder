...
__all__ = ['Contest', 'PastContest', 'ContestTag', 'ContestParticipation', 'ContestProblem', 'ContestSubmission', 'Rating']
...
class PastContest(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name=_('Past contest name'), db_index=True)
    level = models.CharField(verbose_name=_('Contest level'), max_length=7, choices=LEVELS)
    difficulty = models.CharField(verbose_name=_('Contest difficulty'),max_length=7, choices=DIFFICULTIES)
    year = models.PositiveIntegerField()

    class Meta:
        verbose_name = _('past contest')
        verbose_name_plural = _('past contests')

class PastContestProblem(models.Model):

    problem = models.ForeignKey(Problem, verbose_name=_('problem'), related_name='past_contests', on_delete=CASCADE)
    past_contest = models.ForeignKey(PastContest, verbose_name=_('related past contest'), related_name='past_contest_problems', on_delete=CASCADE)

    class Meta:
        unique_together = ('problem', 'past_contest')
        verbose_name = _('past contest problem')
        verbose_name_plural = _('past contest problems')