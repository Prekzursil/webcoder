...
class FirstBlood(MetaBadge):
    id = "first-blood"
    model = Submission
    one_time_only = True

    title = _('First Blood')
    description = _('Submit your first solution')
    level = "1"
    ...
    def get_progress(self, user):
        has_about = 1 if user.profile.about else 0
        return has_about

    def check_bio(self, instance):
        return True
class ProblemSolver(MetaBadge):
    id = "problem-solver"
    model = Submission
    one_time_only = True

    title = _('Problem Solver')
    description = _('Solve your first problem')
    level = "1"
