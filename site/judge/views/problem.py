...
from django.db.models import Count, Max, Value, F, Prefetch, Q
...
from judge.models.problem import ProblemGroupTranslation, TranslatedProblemGroupQuerySet
from judge.models.contest import PastContestProblem
...
from judge.models.choices import ACE_THEMES
...
class ProblemDetail(ProblemMixin, SolvedProblemMixin, CommentedDetailView):
    context_object_name = 'problem'
    template_name = 'problem/problem-modified.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        context['has_max_score'] = False
        if authed and Submission.objects.filter(user=user.profile, problem=self.object).exists():
            for sub in Submission.objects.filter(user=user.profile, problem=self.object):
                if sub.case_points:
                    if sub.case_points/sub.case_total:
                        context['has_max_score'] = True
                        break
        ...
        context['submissions'] = Submission.objects.filter(problem=self.object).order_by('-case_points', 'date')
        if authed:
            context['my_submissions'] = Submission.objects.filter(problem=self.object, user=user.profile).order_by('-case_points', 'date')[0:5]
        try:
        ...
        try:
            ...
            context['description'] = self.object.description
            context['hints'] = self.object.hints
            ...
            context['hints'] = translation.hints
        ...
        solution = Solution.objects.filter(problem=self.object)
        if solution:
            context['solution'] = solution[0]
        if authed:
            context['submit'] = problem_submit_context(self.request, self.object.code, submission=None)
    ...
    def post(self, request, *args, **kwargs):
        ...
        elif 'source' in request.POST.keys():
            self.object = self.get_object()
            return problem_submit_context(request, problem=self.object.code, submission=None)
...
class CategoriesList(QueryStringSortMixin, TitleMixin, SolvedProblemMixin, ListView):
    model = Problem
    title = gettext_lazy('Categories')
    context_object_name = 'problems'
    template_name = 'problem/list-categories.html'
    paginate_by = 50
    ...
    def get_paginator(self, queryset, per_page, orphans=0,
        ...
        if not self.in_contest:
            ...
            queryset = queryset.add_i18n_summary(self.request.LANGUAGE_CODE)
    ...
    def get_contest_queryset(self):
        ...
        queryset = TranslatedProblemForeignKeyQuerySet.add_problem_i18n_name(queryset, 'i18n_name',
                                                                             ...
                                                                             'problem__name')
        queryset = TranslatedProblemForeignKeyQuerySet.add_problem_i18n_summary(queryset, 'i18n_summary',
                                                                             self.request.LANGUAGE_CODE,
                                                                             'problem__summary')
        return [{
            'id': p['problem_id'],
            ...
            'i18n_summary': p['i18n_summary'],
            'group': {'full_name': p['problem__group__full_name']},
            'points': p['points'],
            'partial': p['partial'],
            'user_count': p['user_count'],
        } for p in queryset.values('problem_id', 'problem__code', 'problem__name', 'i18n_name', 'i18n_summary',
    ...
    def get_context_data(self, **kwargs):
        ...
        pgroupqs = ProblemGroup.objects.exclude(name='Uncategorized')
        # pgroupqs = TranslatedProblemGroupQuerySet.add_i18n_full_name(pgroupqs, self.request.LANGUAGE_CODE)
        context['categories'] = pgroupqs
        # groupTranslation = []
        # for category in ProblemGroup.objects.exclude(name='Uncategorized'):
        #     with ProblemGroupTranslation.objects.filter(group=category) as translated:
        #         if translated:
        #             groupTranslation.append(translated.full_text)
        #         else:
        #             groupTranslation.append('')
        # context['translated_categories'] = groupTranslation
        ...
        context['completed_problem_ids'] = self.get_completed_problems()
        context['attempted_problems'] = self.get_attempted_problems()
        context['contest_problems'] = Problem.objects.filter(contests__isnull=False).distinct().count()
...
class ProblemList(QueryStringSortMixin, TitleMixin, SolvedProblemMixin, ListView):
    ...
    context_object_name = 'problems'
    template_name = 'problem/list-modified.html'
    paginate_by = 10
    ...
    def get_contest_queryset(self):
        ...
        for p in queryset:
            context = {}
            context['id'] = p.id
            context['code'] = p.problem.code
            context['name'] = p.problem.name
            context['i18n_name'] = p.i18n_name
            context['i18n_summary'] = p.i18n_summary
            context['authors'] = p.problem.authors
            context['uploader'] = p.problem.uploader
            context['group'] = p.problem.group
            context['types_list'] = p.problem.types.all()
            context['points'] = p.points
            context['partial'] = p.partial
            context['user_count'] = p.user_count
            context['difficulty'] = p.problem.get_difficulty_display()
            context['no_difficulty'] = True

            scores = list(p.problem.submission_set.filter(user=self.request.user.profile).values_list('case_points', flat=True))

            context['max_score'] = max(scores) if scores else None
    ...
    def get_normal_queryset(self):
        ...
        #     queryset = queryset.exclude(id__in=Submission.objects.filter(user=self.profile, points=F('problem__points'))
        #                                 .values_list('problem__id', flat=True))
        if self.profile and (self.solved or self.unsolved):
            if self.solved and not self.unsolved: queryset = queryset.filter(id__in=Submission.objects.filter(user=self.profile, case_total=F('problem__points')).values_list('problem__id', flat=True))
            if not self.solved and self.unsolved: queryset = queryset.exclude(id__in=Submission.objects.filter(user=self.profile, case_total=F('problem__points')).values_list('problem__id', flat=True))
        if self.easy or self.medium or self.hard:
            difficulty = []
            if self.easy: difficulty.append('es')
            if self.medium: difficulty.append('md')
            if self.hard: difficulty.append('hd')
            queryset = queryset.filter(difficulty__in=difficulty)
        if self.contests:
            queryset = queryset.exclude(contests__isnull=True, past_contests__isnull=True)
        if self.show_types:
            queryset = queryset.prefetch_related('types')
        if self.category is not None:
            group = ProblemGroup.objects.get(id=self.category)
            if not group.parent:
                if group.get_subgroups_count() > 0:
                    queryset = queryset.filter(group__parent__id=self.category)
                else:
                    queryset = queryset.filter(group__id=self.category)
            else:
                queryset = queryset.filter(group__id=self.category)
        if self.past_contest is not None:
            past_problems = list(PastContestProblem.objects.filter(past_contest=self.past_contest).values_list('problem', flat=True))
            queryset = queryset.filter(id__in=past_problems)
        if self.language is not None:
            queryset = queryset.filter(allowed_languages__id=self.language)
        if self.selected_types:
            queryset = queryset.filter(types__in=self.selected_types)
        if 'search' in self.request.GET:
            self.search_query = query = ' '.join(self.request.GET.getlist('search')).strip()
            if query:
                if settings.ENABLE_FTS and self.full_text:
                    ...
                    queryset = queryset.filter(
                        Q(code__icontains=query) | Q(name__icontains=query) | Q(allowed_languages__name__icontains=query) |
                        Q(translations__name__icontains=query, translations__language=self.request.LANGUAGE_CODE))
        self.prepoint_queryset = queryset
        if self.point_start is not None:
            queryset = queryset.filter(points__gte=self.point_start)
        ...
        authed = self.request.user.is_authenticated
        if authed:
            queryset = queryset.annotate(max_score=Max('submission__case_points', filter=Q(submission__user=self.request.user.profile)))
        else:
            queryset = queryset.annotate(max_score=Value(None, output_field=CharField()))
    ...
    def get_context_data(self, **kwargs):
        ...
        context['show_types'] = 0 if self.in_contest else int(self.show_types)
        context['full_text'] = 0 if self.in_contest else int(self.full_text)
        context['solved'] = 0 if self.in_contest else int(self.solved)
        context['unsolved'] = 0 if self.in_contest else int(self.unsolved)
        context['easy'] = 0 if self.in_contest else int(self.easy)
        context['medium'] = 0 if self.in_contest else int(self.medium)
        context['hard'] = 0 if self.in_contest else int(self.hard)
        context['contests'] = 0 if self.in_contest else int(self.contests)
        context['category'] = self.category
        context['past_contest'] = self.past_contest
    ...
    def setup_problem_list(self, request):
        ...
        self.full_text = self.GET_with_session(request, 'full_text')
        self.solved = self.GET_with_session(request, 'solved')
        self.unsolved = self.GET_with_session(request, 'unsolved')
        self.easy = self.GET_with_session(request, 'easy')
        self.medium = self.GET_with_session(request, 'medium')
        self.hard = self.GET_with_session(request, 'hard')
        self.contests = self.GET_with_session(request, 'contests')

        self.search_query = None
        self.category = None
        self.language = None
        self.past_contest = None
        self.selected_types = []
        ...
        self.category = safe_int_or_none(request.GET.get('category'))
        self.language = safe_int_or_none(request.GET.get('language'))
        self.past_contest = safe_int_or_none(request.GET.get('past_contest'))
        if 'type' in request.GET:
            try:
                self.selected_types = list(map(int, request.GET.getlist('type')))
            except ValueError:
    ...
    def post(self, request, *args, **kwargs):
        to_update = ('hide_solved', 'show_types', 'full_text', 'solved', 'unsolved', 'easy', 'medium', 'hard', 'contests')
        for key in to_update:
            if key in request.GET:
                val = request.GET.get(key) == '1'
                request.session[key] = val
...
@login_required
def problem_submit_context(request, problem, submission=None):
    if submission is not None and not request.user.has_perm('judge.resubmit_other') and \
            get_object_or_404(Submission, id=int(submission)).user.user != request.user:
        raise PermissionDenied()
