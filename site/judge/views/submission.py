...
class SubmissionsListBase(DiggPaginatorMixin, TitleMixin, ListView):
    ...
    def _get_queryset(self):
        ...
        if self.country:
            queryset = queryset.filter(user__country=self.country)
        if self.order:
            field_names = ['problem', '-problem', 'user', '-user', 'country', '-country',
                           'language', '-language', 'memory', '-memory', 'time', '-time', 'score', '-score']
            valid_names = ['problem__name', '-problem__name', 'user__user__first_name', '-user__user__first_name',
                           'user__country', '-user__country', 'language', '-language', 'memory', '-memory', 'time', '-time', '-case_points', 'case_points']
            if self.order in field_names:
                queryset = queryset.order_by(valid_names[field_names.index(self.order)])
            else:
                pass
...
def single_submission(request, submission_id, show_problem=True):
    ...
    return render(request, 'submission/row-modified.html', {
...
class AllSubmissions(SubmissionsListBase):
    ...
    def _get_result_data(self):
        if self.in_contest or self.selected_languages or self.selected_statuses or self.country:
