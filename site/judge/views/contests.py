...
from judge.models.contest import PastContest
...
class ContestList(DiggPaginatorMixin, TitleMixin, ContestListMixin, ListView):
    model = Contest
    paginate_by = 20
    template_name = 'contest/list-modified.html'
...
class PastContestList(DiggPaginatorMixin, TitleMixin, ListView):
    model = PastContest
    paginate_by = 20
    template_name = 'contest/list-past-contests.html'
    title = gettext_lazy('Past Contests')
    context_object_name = 'past_contests'
    ...
    def _get_queryset(self):
        return super(PastContestList, self).get_queryset()

    def get_queryset(self):
        return self._get_queryset()

    def get_context_data(self, **kwargs):
        context = super(PastContestList, self).get_context_data(**kwargs)
        context['years'] = PastContest.objects.all().order_by('-year').values_list('year', flat=True).distinct()
        return context
